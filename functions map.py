...
# num_list = list(map(lambda x, y, w: x ** 2 - x * y * w + w ** 4, list_x, list_y, list_w))
# print(num_list)
...
# days_filtr = list(filter(lambda x: len(x)== 4 or x[0] == 'S', days))
# print(*(sorted(days_filtr)), sep='\n')

####
# Напишите определение функции zip_with_function
# def zip_with_function(l: list, f) -> list:
#     l_columns = list(zip(*l))
#     x = [f(*args) for args in l_columns]
#     return x
#
#
...
#     """ функцию , которая принимает произвольное количество аргументов.
...
#             a.append(k)
#     return sorted(a, key=lambda i: i.lower())
#
...
# print(word_list)
# print(all(['a' in word for word in word_list]))

###
word_list = input().lower().split()
print(any([word.endswith("ought") for word in word_list]))
